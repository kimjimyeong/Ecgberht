    ...
    public State execute() {
        try {
            if (!gameState.Fs.isEmpty()) {
                if (gameState.getStrategyFromManager().getName().equals("VultureRush") || Util.countUnitTypeSelf(UnitType.Terran_Vulture) * 2 <= Util.countUnitTypeSelf(UnitType.Terran_Siege_Tank_Siege_Mode) + Util.countUnitTypeSelf(UnitType.Terran_Siege_Tank_Tank_Mode) + 2) {
