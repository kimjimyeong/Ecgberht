...
public class ChooseSCV extends Action {
    ...
    public State execute() {
        try {
            ...
            if (Util.countUnitTypeSelf(UnitType.Terran_SCV) <= 65 && Util.countUnitTypeSelf(UnitType.Terran_SCV) <= gameState.mineralsAssigned.size() * 2 + gameState.refineriesAssigned.size() * 3 + gameState.getStrategyFromManager().getExtraSCVs() && !gameState.CCs.isEmpty()) {
